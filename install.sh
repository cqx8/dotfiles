...
# 检查是否在中国大陆,并设置全局代理前缀
is_china=$(curl -fsSL -m 5 https://ip.seiya.dev/country 2>/dev/null | grep -Eq '^(CN|HK)$' && echo 1 || echo 0)
...
# 如果在中国, 为 vim-plug 创建本地加速配置
if [[ "$is_china" -eq 1 ]]; then
    VIMRC_LOCAL_PATH="${HOME}/.vimrc.local"
    PLUG_URL_LINE="let g:plug_url_format = '${PREFIX_PROXY}https://github.com/%s.git'"

    info "⚡️ 检测到在中国, 配置 vim-plug 加速."
    touch "$VIMRC_LOCAL_PATH"
    
    if ! grep -qF -- "$PLUG_URL_LINE" "$VIMRC_LOCAL_PATH"; then
        info "  正在写入加速源到 .vimrc.local ..."
        echo "$PLUG_URL_LINE" >> "$VIMRC_LOCAL_PATH"
        info "✅ 已将 vim-plug 加速源写入 .vimrc.local"
    else
        info "✅ vim-plug 加速源已存在于 .vimrc.local, 跳过."
    fi
fi
